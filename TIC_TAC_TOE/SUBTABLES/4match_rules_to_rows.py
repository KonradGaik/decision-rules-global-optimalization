...
for i in range(1,6):
    ...
    csv_file_data = os.path.join(f"./subtable_{i}", f"consistent_modified_tic-tac-toe{i}.csv")
    csv_file_rules = os.path.join(f"./subtable_{i}", f"decision_rules{i}.csv")
    ...
    output_file = os.path.join(f'./subtable_{i}', f"matched_rows{i}.csv")
