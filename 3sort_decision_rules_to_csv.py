...
def get_rules(tree, feature_names, class_names):
    ...
    def recurse(node, path, paths):
        ...
        if tree_.feature[node] != _tree.TREE_UNDEFINED:
            ...
            paths.append((rule, tree_.n_node_samples[node]))  # Dodanie liczby próbek do ścieżki
    ...
    return [path[0] for path in paths]  # Zwrócenie posortowanych reguł
...
for i, file_name in enumerate(sorted(os.listdir(folder_name)), start=1):
    if file_name.endswith(".csv"):
        ...
        output_file = os.path.join(output_folder, f"decision_rules_{i}_sort.csv")
