...
def process_csv_file(csv_file, index):
    ...
    df_rules = pd.DataFrame(columns=["Decision"] + list(X.columns))
    for i, rule in enumerate(rules, start=1):
        descriptors = rule.split(" & ")
        decision = descriptors[-1].split(" => ")[1]
        descriptors = descriptors[:-1]
        rule_dict = {"Decision": decision}
        for descriptor in descriptors:
            column_name, condition = descriptor.split(" > ")
            rule_dict[column_name] = condition
        df_rules = df_rules.append(rule_dict, ignore_index=True)
