...
def process_csv_file(csv_file, index, output_folder):
    ...
    with open(output_file_rules_txt, 'w') as f_txt, open(output_file_rules_csv, 'w', newline='') as f_csv:
        ...
        for rule_index, rule in enumerate(terminal_rules):
            ...
            if not matched_rows.empty:
                ...
                # Dodawanie kolumn do `matched_rows` dla `Decision_Class`, `Rule_Length` i `Rule`
                matched_rows['Decision_Class'] = decision
                matched_rows['Rule_Length'] = rule_length
                matched_rows['Rule'] = conditions

                # Dołączamy kolumnę `Order` z `test_df`
                matched_rows = pd.concat([test_df.loc[matched_rows.index, ['Order']], matched_rows], axis=1)
                
                # Filtracja dopasowanych wierszy tylko tam, gdzie `Class == Decision_Class`
                filtered_matched_rows = matched_rows[matched_rows['Class'] == matched_rows['Decision_Class']]
                
                # Zapisanie przefiltrowanych wierszy do pliku CSV
                matched_rows_file = os.path.join(output_folder, f"rule_{index}_{rule_index+1}.csv")
                filtered_matched_rows.to_csv(matched_rows_file, index=False)
    ...
    classification_results_file = os.path.join(f'../RESULTS/subtable_{index}', f"classification_results_{index}.csv")
...
#                 # Dołączamy kolumnę `Order` z `test_df` i unikamy duplikowania kolumn
#                 matched_rows = pd.concat([test_df.loc[matched_rows.index, ['Order']], matched_rows], axis=1)
                
#                 # Zapisanie dopasowanych wierszy do pliku CSV
#                 matched_rows_file = os.path.join(output_folder, f"rule_{index}_{rule_index+1}.csv")
