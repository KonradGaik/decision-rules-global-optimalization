...
for x in range(0, 5):
  print(x)
  if not os.path.exists(f'../RESULTS/subtable_{x+1}'):
    os.makedirs(f'../RESULTS/subtable_{x+1}')



for i in range(1, 6):
    csv_file = f"../RESULTS/subtable_{i}/2encoded_tic_tac_toe_{i}.csv"
    decision_grouped = replace_inconsistencies(csv_file)
    save_to_csv(decision_grouped, f'../RESULTS/subtable_{i}', f"3consistent_encoded_modified_tic-tac-toe{i}.csv")
